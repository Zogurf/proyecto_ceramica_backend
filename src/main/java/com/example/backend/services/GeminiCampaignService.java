...
public class GeminiCampaignService {
    ...
    public CampaignResponse previewCampaign(CampaignRequest request) {
        CampaignAudience campaignAudience = buildCampaignAudience(request);
        String subject = resolveSubject(request, campaignAudience.product());
        String htmlTemplate = buildStoreTemplate();

        return new CampaignResponse(campaignAudience.audience().size(), subject, htmlTemplate);
    }

    public CampaignResponse sendCampaign(CampaignRequest request) {
        CampaignAudience campaignAudience = buildCampaignAudience(request);
        Product product = campaignAudience.product();
        String subject = resolveSubject(request, product);
        String htmlTemplate = buildStoreTemplate();

        campaignAudience.audience().values().forEach(user -> {
    ...
    private CampaignAudience buildCampaignAudience(CampaignRequest request) {
        ...
        return new CampaignAudience(product, audience);
    }

    private String resolveSubject(CampaignRequest request, Product product) {
        return StringUtils.hasText(request.subject())
                ? request.subject()
                ...
                <div style="background:#f6efe8;padding:28px 16px;font-family:Arial,Helvetica,sans-serif;color:#2d211b;">
                  <div style="max-width:560px;margin:0 auto;background:#fffdfb;border:1px solid #eadbd0;border-radius:20px;overflow:hidden;">
                    <div style="background:#2d211b;padding:24px;">
                      <p style="margin:0;color:#d9b49d;font-size:12px;font-weight:700;letter-spacing:1px;text-transform:uppercase;">El mundo de Mery</p>
                      <h1 style="margin:8px 0 0;color:white;font-size:28px;">{{emailTitle}}</h1>
                    </div>
                    <div style="padding:24px;">
                      <p style="font-size:16px;line-height:1.7;margin:0 0 16px;">Hola {{customerName}}, vimos que te intereso <strong>{{productName}}</strong> y preparamos una oferta especial para ti.</p>
    ...
    private record CampaignAudience(Product product, Map<String, User> audience) {
    }
}
